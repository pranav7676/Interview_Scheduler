...
const InterviewDetails: React.FC<InterviewDetailsProps> = ({ selectedDate }) => {
  ...
  // Show data only if selected date is from current or future months
  const interviews = isSameOrAfterCurrentMonth
    ? data
        .filter((item) => {
          const interviewDate = new Date(item.datetime);
        ...
        .map((item) => ({
          ...
          image: item.candidate.avatar,
        }))
    : []; // ❌ show nothing for previous months
  ...
  return (
    <div className="bg-white rounded-md shadow p-4 w-full max-w-md">
      ...
      <div
        className={`space-y-3 max-h-[320px] ${
          interviews.length > 0 ? "overflow-y-auto pr-1" : ""
        }`}
      >
        {interviews.length === 0 && (
        ...
        <AnimatePresence initial={false}>
          {interviews.map((item, index) => (
            <motion.div
              ...
              className="flex items-center justify-between rounded px-4 py-2 bg-blue-50"
            >
              ...
              <div className="text-sm text-gray-500 whitespace-nowrap">At {item.time}</div>
            </motion.div>
          ))}
        </AnimatePresence>
      </div>
